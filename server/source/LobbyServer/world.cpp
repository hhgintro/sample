...
void CWorld::Clear()
{
	...
	m_nStatus = NetCore::S_STATUS::eS_Stop;
}
...
DWORD CWorld::InitializeNetworkServer()
{
	...
	dwResult = server_.Start(CParam::Instance().max_user
		...
		, 32//ServerRecvUdpPoolCount_
		, 32//ServerSendUdpPoolCount_
		, 81960//RemoteRecvTcpPoolSize_
		, 32//RemoteRecvUdpPoolCount_
	);
...
DWORD CWorld::InitializeNetworkConnect()
{
	...
	master_.SetRemoteEntry(REMOTE_ENTRY::eLobbyServer);
	master_.BeginClient(CParam::Instance().ipMaster
		, CParam::Instance().portMaster
		, CParam::Instance().portMaster
		, 81960//ClientRecvTcpPoolSize_
		, 32//ClientRecvUdpPoolCount_
		, 32//ClientSendUdpPoolCount_)
	);
