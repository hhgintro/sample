...
/////*//HG_TEST : 테스트를 위해서 중복 체크를 하지 않는다.
	...
	////	//프로토콜 버전(game)
	//if (PROTOCOL_VERSION != st.protocol)
	//{
	//	stX2G_AC_LOGIN_ACK stAck;
	//	stAck.result = ERR_DIFFERENCE_PROTOCOL_VERSION;
	//	SEND_X2G_AC_LOGIN_ACK(session->GetHostID(), NetCore::RELIABLE, stAck);
	//	//return;//HG_TEST
	//}

	////중복 체크
	//if (NULL != CAccountManager::Instance().Find(st.account))
	//{
	//	stX2G_AC_LOGIN_ACK stAck;
	//	stAck.result = ERR_DUPLICATE_ACCOUNT;
	//	SEND_X2G_AC_LOGIN_ACK(session->GetHostID(), NetCore::RELIABLE, stAck);
	//	//return;//HG_TEST
	//}
