...
int CParam::Read()
{
	...
	//ip = wzIP;
	ip_master = HisUtil::IP_wTon(wzIP);

	//Listen socket port
	if (FALSE == loader_.GetValue(L"master", L"port", port_master))
		return MASTER_PORT_INVALID;
	port_master = htons(port_master);

	//client pool
	loader_.GetValue(L"master", L"ClientRecvTcpPoolSize", ClientRecvTcpPoolSize);
	loader_.GetValue(L"master", L"ClientRecvUdpPoolCount", ClientRecvUdpPoolCount);
	loader_.GetValue(L"master", L"ClientSendUdpPoolCount", ClientSendUdpPoolCount);
