...
/*
	...
	WCHAR buf[size+1] = L"";
	memset(buf, '8', size*2);
	stM2_BD_VOICE_CHAT_REQ stReq;
	stReq.chat = buf;
	SEND_M2_BD_VOICE_CHAT_REQ(NetCore::Host_Server, NetCore::RELIABLE, stReq);
*/
}
...
VOID CMasterClient::OnIoRecv(HisUtil::CMsg& msgRecv)
{
	// 프로토콜에 따른 switch 문
	switch (msgRecv.ID())
	{
		////tcp
		ON_RECEIVE_ON_CLIENT(M2_BD_VOICE_CHAT_ACK);

		ON_RECEIVE_ON_CLIENT(M2_AC_LOGIN_ACK);

		ON_RECEIVE_ON_CLIENT(M2_IN_EQUIP_LOAD_ACK);
		ON_RECEIVE_ON_CLIENT(M2_IN_EQUIP_UPDATE_ACK);
...
////}
////
////CPacketSession* CMasterClient::FindPacketSession(NetCore::IRemote* peer, bool bIsUdp)
////{
////	if(bIsUdp)
