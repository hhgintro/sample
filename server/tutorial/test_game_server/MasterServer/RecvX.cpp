...
////	{
////		LOGEVENT("Session_Base_Error", _T("Error 접속이 종료된 개체입니다. : 0x%08x"), st.remote);
////		return;
////	}
////
...
//
//
void CServer::OnM2_BD_VOICE_CHAT_REQ(NetCore::CRemoteSession* session, HisUtil::CMsg& msgRecv)
{
	READ_PACKET(M2_BD_VOICE_CHAT_REQ);

	stM2_BD_VOICE_CHAT_ACK stAck;
	stAck.chat = st.chat;
	SEND_M2_BD_VOICE_CHAT_ACK(session->GetHostID(), NetCore::RELIABLE, stAck);
}
