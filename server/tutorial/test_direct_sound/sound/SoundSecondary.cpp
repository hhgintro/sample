...
SoundSecondary::SoundSecondary()
	: m_pWavData(NULL), m_timerID(0), SOUND_BUFFER(0)
{
	init();
}
...
void SoundSecondary::clear(DWORD buflen)
{
	SOUND_BUFFER = buflen;
	m_poolData.Clear();
}
...
bool SoundSecondary::CreateSoundBuffer(CComPtr<IDirectSound8> & directSound)
{
	...
	bufferDesc.dwFlags			= DSBCAPS_CTRLVOLUME | DSBCAPS_STATIC | DSBCAPS_GLOBALFOCUS | DSBCAPS_CTRLPOSITIONNOTIFY;
...
void SoundSecondary::AddStreamBuffer()
{
	...
	for (int i = 0; i < poolCnt; ++i)
	{
		...
		if(NULL == sound) continue;

		sound->Clear(SOUND_BUFFER);
